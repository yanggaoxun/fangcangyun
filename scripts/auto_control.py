...
控制类型:
    temperature     - 温度控制（制冷/加热）- 支持 threshold/schedule/cycle
...
# 各控制类型支持的模式
CONTROL_MODES = {
    'temperature': ['auto_threshold', 'auto_schedule', 'auto_cycle'],
...
class AutoControl:
    ...
    def process_threshold_mode(self, control_type, control_config):
        ...
        逻辑:
        - temperature: 高于上限开制冷，低于下限开加热
        - humidity: 低于下限开加湿，高于上限关加湿
        - fresh_air/exhaust: CO2高于上限开，低于下限关
        """
        # 光照不支持阈值模式
    ...
    def process_schedule_mode(self, control_type, control_config):
        ...
        逻辑:
        - 检查当前时间是否在设定的时段内
        - temperature: 时段内再根据温度阈值判断
        - humidity: 时段内再根据湿度阈值判断
        - lighting: 时段内使用 LED 开/关时长循环控制
        - fresh_air/exhaust: 时段内根据CO2阈值判断
        """
        ...
        if control_type == 'lighting':
            ...
            if cooling_upper and temp > cooling_upper:
                logger.info(f"【温度定时】{temp}°C > 制冷上限 {cooling_upper}°C，开启制冷")
                self.controller.set_state('cooling', True)
                self.controller.set_state('heating', False)
            elif heating_lower and temp < heating_lower:
                logger.info(f"【温度定时】{temp}°C < 加热下限 {heating_lower}°C，开启加热")
                self.controller.set_state('cooling', False)
                self.controller.set_state('heating', True)
            else:
                logger.info(f"【温度定时】{temp}°C 在设定范围内，关闭温控")
                self.controller.set_state('cooling', False)
                self.controller.set_state('heating', False)
