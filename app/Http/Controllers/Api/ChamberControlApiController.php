...
namespace App\Http\Controllers\Api;
            ...
            class ChamberControlApiController extends Controller
            {
                ...
                public function manualControl(Request $request, Chamber $chamber): JsonResponse
                {
                    ...
                    // 通过 MQTT 下发命令到边缘设备
                    try {
                        $commandId = \App\Services\MqttPublisher::publishManualControl(
                            $chamber->code,
                            $updateData,
                            $request->input('override_minutes')
                        );

                        return response()->json([
                            'success' => true,
                            'message' => '设备状态已更新，命令已下发到边缘设备',
                            'data' => [
                                'chamber_id' => $chamber->id,
                        ...
                        // 即使 MQTT 失败，数据库更新仍然成功
                        return response()->json([
                            'success' => true,
                            'message' => '设备状态已更新，但命令下发失败',
                            'warning' => $e->getMessage(),
                            'data' => [
                                'chamber_id' => $chamber->id,
                                'updated_fields' => $updateData,
