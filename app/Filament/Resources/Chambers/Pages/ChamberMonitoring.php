...
class ChamberMonitoring extends Page implements HasTable
{
    ...
    {
        $record = EnvironmentData::where('chamber_id', $chamberId)->latest('recorded_at')->first();
        ...
        $deviceNames = EnvironmentData::getDeviceNames();
    ...
    {
        return $table
            ->query(EnvironmentData::query()->with(['chamber.base', 'chamber']))
            ->columns([
            ...
            ->filters([
                Filter::make('base_id')
                    ...
                    ->form([
                        Select::make('value')
                            ->label('基地')
                            ->options(fn (): array => Base::where('status', 'active')->pluck('name', 'id')->toArray())
                            ->searchable()
                            ->preload()
                            ->live()
